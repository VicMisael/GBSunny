...
void cpu::cpu::gb_doctor_print(std::ostream& out_stream)
{
    ...
    out_stream << "PC:" << std::setw(4) << current_pc << " ";

    // Print memory at the Program Counter
    out_stream << "PCMEM:";
    for (int i = 0; i < 4; ++i) {
        out_stream << std::setw(2) << static_cast<int>(_mmu->read((current_pc) + i));
        if (i < 3) {
            out_stream << ",";
        }
    }
...
uint32_t cpu::cpu::step() {
    ...
    if ( print && !_mmu->boot_rom_enabled())
        _registers.print_registers();

    decoded_instruction instruction{.opcode =_mmu->read(_registers.pc++) };

    if (print && !_mmu->boot_rom_enabled()) {
        std::cout << (opcode_names[instruction.opcode]) << std::endl;
