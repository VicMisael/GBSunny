...
SPU2::SPU2(std::shared_ptr<shared::interrupt> interrupts)
	: interrupts(std::move(interrupts))
{
	reset_audio_registers();
}

void SPU2::reset_audio_registers()
{
	registers.reset();
	channel1 = {};
	channel2 = {};
...
uint8_t SPU2::read(uint16_t addr) const
{
	return registers.read(addr, powered_on, channel_status());
}

uint8_t SPU2::read_wave(uint16_t addr) const
{
	return registers.read_wave(addr);
}

void SPU2::write(uint16_t addr, uint8_t data)
{
	if (addr == 0xFF26) {
		if ((data & 0x80) == 0) {
			power_off();
...
void SPU2::write_wave(uint16_t addr, uint8_t data)
{
	registers.write_wave(addr, data);
}

void SPU2::step(uint32_t cycles)
{
	while (cycles > 0) {
		const uint32_t cycles_to_frame_sequencer =
			gb_hardware::apu::FrameSequencerPeriod - frame_sequencer_cycles;
		const uint32_t cycles_to_sample =
			(gb_hardware::CpuClockHz - sample_clock_accumulator
				+ gb_hardware::apu::SampleRate - 1)
			/ gb_hardware::apu::SampleRate;
		const uint32_t advance = std::min(
			cycles,
			std::min(cycles_to_frame_sequencer, cycles_to_sample));

		if (powered_on) {
			advance_channels(advance);
		}

		frame_sequencer_cycles += advance;
		...
		sample_clock_accumulator += advance * gb_hardware::apu::SampleRate;
...
void SPU2::tick_frame_sequencer()
{
	if (powered_on) {
...
void SPU2::tick_length_counters()
{
...
void SPU2::trigger_envelope(envelope_state& envelope, uint8_t nrx2)
{
...
void SPU2::tick_envelope(envelope_state& envelope)
{
	if (!envelope.running) {
		return;
	}
...
uint16_t SPU2::calculate_sweep_period(bool update)
{
	const uint8_t sweep = registers.channel1.sweep();
...
void SPU2::tick_pulse(pulse_channel_state& channel, uint16_t period_value)
{
	if (!channel.enabled) {
		return;
	}
...
void SPU2::advance_channels(uint32_t cycles)
{
	advance_pulse(channel1, channel1_period(), cycles);
	advance_pulse(channel2, channel2_period(), cycles);
	advance_wave(cycles);
	advance_noise(cycles);
}

void SPU2::advance_pulse(
	pulse_channel_state& channel,
	uint16_t period_value,
	uint32_t cycles)
{
	if (!channel.enabled || cycles == 0) {
		return;
	}

	const uint32_t timer_period = static_cast<uint32_t>((2048 - period_value) * 4);
	if (channel.period_timer == 0) {
		const uint32_t events = 1 + (cycles - 1) / timer_period;
		channel.duty_position = static_cast<uint8_t>(
			(channel.duty_position + events) & 0x07);
		channel.period_timer = timer_period - ((cycles - 1) % timer_period);
		return;
	}

	if (channel.period_timer > cycles) {
		channel.period_timer -= cycles;
		return;
	}

	const uint32_t remaining_cycles = cycles - channel.period_timer;
	const uint32_t events = 1 + remaining_cycles / timer_period;
	channel.duty_position = static_cast<uint8_t>(
		(channel.duty_position + events) & 0x07);
	channel.period_timer = timer_period - (remaining_cycles % timer_period);
}

void SPU2::advance_wave(uint32_t cycles)
{
	if (!channel3.enabled || cycles == 0) {
		return;
	}

	const uint32_t timer_period = static_cast<uint32_t>((2048 - channel3_period()) * 2);
	if (channel3.period_timer == 0) {
		const uint32_t events = 1 + (cycles - 1) / timer_period;
		channel3.sample_position = static_cast<uint8_t>(
			(channel3.sample_position + events) & 0x1F);
		channel3.period_timer = timer_period - ((cycles - 1) % timer_period);
		return;
	}

	if (channel3.period_timer > cycles) {
		channel3.period_timer -= cycles;
		return;
	}

	const uint32_t remaining_cycles = cycles - channel3.period_timer;
	const uint32_t events = 1 + remaining_cycles / timer_period;
	channel3.sample_position = static_cast<uint8_t>(
		(channel3.sample_position + events) & 0x1F);
	channel3.period_timer = timer_period - (remaining_cycles % timer_period);
}

void SPU2::advance_noise(uint32_t cycles)
{
	if (!channel4.enabled || cycles == 0) {
		return;
	}
	...
	if (channel4.period_timer > cycles) {
		channel4.period_timer -= cycles;
		return;
	}

	uint32_t remaining_cycles = cycles - channel4.period_timer;
	clock_lfsr();

	const uint32_t additional_events = remaining_cycles / timer_period;
	for (uint32_t event = 0; event < additional_events; ++event) {
		clock_lfsr();
	}

	channel4.period_timer = timer_period - (remaining_cycles % timer_period);
}

void SPU2::trigger_channel1()
{
...
uint16_t SPU2::channel1_period() const
{
...
uint16_t SPU2::channel2_period() const
{
	return static_cast<uint16_t>(
...
uint16_t SPU2::channel3_period() const
{
	return static_cast<uint16_t>(
...
uint32_t SPU2::channel4_timer_period() const
{
...
bool SPU2::envelope_dac_enabled(uint8_t nrx2)
{
	return (nrx2 & 0xF8) != 0;
}

uint8_t SPU2::pulse_output(
...
uint8_t SPU2::wave_output() const
{
...
uint8_t SPU2::noise_output() const
{
...
uint8_t SPU2::channel_status() const
{
...
float SPU2::high_pass(float input, float& capacitor) const
{
...
std::vector<SPU2::stereo_sample> SPU2::consume_samples()
{
